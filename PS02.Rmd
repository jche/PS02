...
Clearly, houses that are in better condition are more expensive. (33 is most likely a data input error, NA may even be partially due to purposeful non-reporting)
...
Ultimately, we choose to just use `full_sq`.
...
```{r}
smooth.spline(train$full_sq, train$price_doc, df=5) %>% 
  broom::augment() %>% 
  ggplot(aes(x=x)) +
  geom_point(aes(y=y)) +
...
smooth.spline(train2$full_sq, train2$price_doc, df=5) %>% 
  broom::augment() %>% 
  ggplot(aes(x=x)) +
  geom_point(aes(y=y)) +
  geom_line(aes(y=.fitted), col="blue", size=1)
...
By just qualitatively examining different degrees of freedom, it seems like df=6ish is good.


# Create Submission File

```{r}
model <- smooth.spline(train2$full_sq, train2$price_doc, df=5)
